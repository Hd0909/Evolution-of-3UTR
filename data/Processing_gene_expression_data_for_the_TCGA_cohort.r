...
library(ggsignif)
library(cocor)
source('/media/huangdan/hardisk0/HD/HD_promoter_evolution/program/R_script/functions_evolution.r')
...
get_data1<-function(file,n){
  ...
  if(n==4){cancer_in_data=log2(1e+6*cancer_in_data+1)}
...
get_all_data_independent<-function(cancers,genes_infor){
  ...
  for(cancer in cancers){
    exp_data_temp=get_data1(paste("/media/huangdan/hardisk0/HD/HD_promoter_evolution/primary_data/expression_data/",cancer,"_merged_select_transcript_TPM_scaled.txt",sep=""),4)
